...
from ali.streams import create_mnist_all_data_streams
...
def create_model_brick():
    ...
    ali.initialize()
    raw_marginals, = next(
        create_mnist_all_data_streams(500, 500)[0].get_epoch_iterator())
...
def create_main_loop(save_path, backup_path):
    ...
    streams = create_mnist_all_data_streams(BATCH_SIZE, MONITORING_BATCH_SIZE)
    ...
    extensions = [
        ...
        PlotLoss(backup_path, "ALI MNIST", every_n_epochs=5, after_training=True),
        PlotAccuracy(backup_path, "ALI MNIST", every_n_epochs=5, after_training=True),
        ProgressBar(),
        Printing(),
    ]
...
if __name__ == "__main__":
    name = "ali_mnist_all_04dropout_%dlat" % NLAT
