...
def create_model_brick():
    ...
    ali.initialize()
    raw_marginals, = next(
        create_handbags_shoes_data_streams(500, 500)[0].get_epoch_iterator())
...
def create_models():
    ...
    model = _create_model(with_dropout=True)
...
def create_main_loop(save_path, backup_path):
    ...
    streams = create_handbags_shoes_data_streams(BATCH_SIZE,
    ...
    extensions = [
        ...
        Checkpoint(save_path, after_epoch=True, after_training=True,
                   use_cpickle=True),
        BackupModel(backup_path, save_path, every_n_epochs=10, after_training=True),
        PlotLoss(backup_path, "ALI Handbags vs. Shoes", every_n_epochs=10, after_training=True),
        PlotAccuracy(backup_path, "ALI Handbags vs. Shoes", every_n_epochs=10, after_training=True),
        ProgressBar(),
        Printing(),
    ]
...
if __name__ == "__main__":
    name = "ali_handbags_shoes_celeba_"+str(dropout).replace('.','') + "dropoutGD_"+ str(NLAT)+"lat"
    logging.basicConfig(level=logging.INFO)
    sys.stdout = Logger(filename=os.path.join("MIRIAM_%s_%s.log" % (name, datetime.datetime.now().strftime("%Y-%m-%d_%H%M"))))
    parser = argparse.ArgumentParser(description="Train ALI on Handbags vs. Shoes %s" % name)
